...
for build_system in "$@"; do
  ...
  if [[ $os == "windows" ]]; then
      mv lightzd "$destinationPath"/lightzd.exe
      mv lightzcli "$destinationPath"/lightzcli.exe
  else
      mv lightzd "$destinationPath"/lightzd
      mv lightzcli "$destinationPath"/lightzcli
  fi
done
