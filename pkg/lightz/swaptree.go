...
func (tree *SwapTree) Init(
	...
	tree.ourKey = ourKey
	tree.lightzKey = lightzKey

	var scriptRoot []byte

	if tree.isLiquid {
		tree.liquidIndexed = taproot.AssembleTaprootScriptTree(
			liquidLeaf(tree.ClaimLeaf),
			liquidLeaf(tree.RefundLeaf),
		)
		h := tree.liquidIndexed.RootNode.TapHash()
		scriptRoot = h[:]
	} else {
		tree.indexed = txscript.AssembleTaprootScriptTree(tree.ClaimLeaf, tree.RefundLeaf)
		h := tree.indexed.RootNode.TapHash()
		scriptRoot = h[:]
	}

	// lightzd key always comes first
	keys := []*btcec.PublicKey{lightzKey, ourKey.PubKey()}
...
func (tree *SwapTree) Address(network *Network, blindingPubKey *btcec.PublicKey) (string, error) {
	...
	if tree.isLiquid {
		...
		address, err := chainutil.NewAddressTaproot(toXOnly(key), network.Btc)
		if err != nil {
			return "", err
		}
...
func (tree *SwapTree) GetControlBlock(isRefund bool) ([]byte, error) {
	...
	if tree.isLiquid {
		var h btcchainhash.Hash
		copy(h[:], leafHash[:])
		idx := tree.liquidIndexed.LeafProofIndex[h]
...
func liquidLeaf(leaf txscript.TapLeaf) taproot.TapElementsLeaf {
	return taproot.TapElementsLeaf{
		TapLeaf: btctxscript.TapLeaf{
			LeafVersion: btctxscript.TapscriptLeafVersion(leaf.LeafVersion),
			Script:      leaf.Script,
		},
	}
}
