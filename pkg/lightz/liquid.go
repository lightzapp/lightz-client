...
func liquidTaprootHash(transaction *liquidtx.Transaction, network *Network, outputs []OutputDetails, index int, cooperative bool) []byte {
	var leafHash *btcchainhash.Hash
	if !cooperative {
	...
	genesisHash, _ := btcchainhash.NewHashFromStr(network.Liquid.GenesisBlockHash)
	...
	// TODO: since we are using go-elemements that use chainhash.Hash of btcsuite instead of chainhash.Hash of github.com/flokiorg/go-flokicoin/chaincfg/chainhash, we need to convert the genesis hash to a chainhash.Hash
	hash := transaction.HashForWitnessV1(
		index,
		scripts,
		assets,
		values,
		btctxscript.SigHashType(sigHashType),
		genesisHash,
		leafHash,
		nil,
	)
