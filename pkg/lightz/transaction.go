...
func ConstructTransaction(network *Network, currency Currency, outputs []OutputDetails, fee Fee, lightzApi *Api) (Transaction, Results, error) {
	...
	for i, output := range outputs {
		err = func() error {
			...
			if lightzApi == nil {
				return errors.New("lightzApi is required for cooperative transactions")
			}
			...
			case ReverseSwap:
				signature, err = lightzApi.ClaimReverseSwap(output.SwapId, claimRequest)
			case NormalSwap:
				signature, err = lightzApi.RefundSwap(output.SwapId, refundRequest)
			default:
				signature, err = func() (*PartialSignature, error) {
					if output.IsRefund() {
						return lightzApi.RefundChainSwap(output.SwapId, refundRequest)
					}
					if output.RefundSwapTree == nil {
						return nil, errors.New("RefundSwapTree is required for cooperatively claiming chain swap")
					}
					session, err := NewSigningSession(output.RefundSwapTree)
					if err != nil {
						return nil, fmt.Errorf("could not initialize signing session: %w", err)
					}
					details, err := lightzApi.GetChainSwapClaimDetails(output.SwapId)
					if err != nil {
						return nil, err
					}
					signature, err := session.Sign(details.TransactionHash, details.PubNonce)
					if err != nil {
						return nil, fmt.Errorf("could not sign transaction: %w", err)
					}
					return lightzApi.ExchangeChainSwapClaimSignature(output.SwapId, &ChainSwapSigningRequest{
						Preimage:  output.Preimage,
						Signature: signature,
						ToSign:    claimRequest,
					})
			...
			if err != nil {
				return fmt.Errorf("could not get partial signature from lightzd: %w", err)
			}
	...
	if reconstruct {
		transaction, newResults, err := ConstructTransaction(network, currency, valid, fee, lightzApi)
		if err != nil {
			return nil, nil, err
		}
