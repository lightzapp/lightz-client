...
import (
	...
	"github.com/lightzapp/lightz-client/internal/onchain/liquid-wallet/lwk"
...
func convertNetwork(network *lightz.Network) *lwk.Network {
	if network == nil {
		return nil
	}
	switch network {
...
func newSigner(network *lightz.Network, mnemonic string) (*lwk.Signer, error) {
...
func DeriveDefaultDescriptor(network *lightz.Network, mnemonic string) (string, error) {
...
func (w *Wallet) autoConsolidate() error {
	...
	if len(utxos) >= int(w.backend.cfg.ConsolidationThreshold) {
		...
		feeRate = max(onchain.FeeFloor[lightz.CurrencyLiquid], feeRate)
...
func GenerateMnemonic(network *lightz.Network) (string, error) {
