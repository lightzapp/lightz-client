...
func TestEstimateFee(t *testing.T) {
	...
	tests := []struct {
		...
		{currency: lightz.CurrencyBtc, fee: 1.0, name: "BTC", expected: 2.0},
		{currency: lightz.CurrencyBtc, fee: 4.0, name: "BTC", expected: 4.0},
		{currency: lightz.CurrencyLiquid, fee: 3.0, name: "LBTC", expected: 3.0},
		{currency: lightz.CurrencyLiquid, fee: 0.0001, name: "LBTC", expected: 0.1},
	}
...
func TestEstimateFee(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			primaryFee:  1.0,
			hasFallback: true,
			expectedFee: onchain.FeeFloor[lightz.CurrencyBtc],
			errorFunc:   require.NoError,
		},
		{
			name:        "Liquid primary success above minimum",
			currency:    lightz.CurrencyLiquid,
			primaryFee:  0.5,
			hasFallback: true,
			expectedFee: 0.5,
			errorFunc:   require.NoError,
		},
		{
			...
			primaryFee:  0.05,
			hasFallback: true,
			expectedFee: onchain.FeeFloor[lightz.CurrencyLiquid],
			errorFunc:   require.NoError,
		},
		{
		...
		},
		{
			...
			currency:    lightz.Currency("invalid"),
			hasFallback: true,
			errorFunc:   require.Error,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.currency != lightz.Currency("invalid") {
...
func TestWalletSync(t *testing.T) {
	...
	setup := func(t *testing.T) *onchain.Onchain {
		onchainInstance := &onchain.Onchain{
			...
			WalletSyncIntervals: map[lightz.Currency]time.Duration{
				lightz.CurrencyBtc:    syncInterval,
				lightz.CurrencyLiquid: syncInterval,
			},
		}
	...
	t.Run("Remove", func(t *testing.T) {
		...
		wallet.EXPECT().GetWalletInfo().Return(onchain.WalletInfo{Id: 1, Currency: lightz.CurrencyBtc}).Maybe()
	...
	t.Run("Disconnect", func(t *testing.T) {
		...
		select {
		case <-done:
		case <-time.After(3 * onchainInstance.WalletSyncIntervals[lightz.CurrencyBtc]):
