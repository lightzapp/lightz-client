...
func NewBackend(cfg Config) (*Backend, error) {
	...
	if cfg.Electrum == nil && cfg.Network == lightz.MainNet {
		electrum := &onchain.ElectrumOptions{
			Url: "esplora.bol.tz:50002",
			SSL: true,
		}
		logger.Infof("BTC: wallet backend fallback: %s", electrum)
		electrumServers = append(electrumServers, electrum)
	}
	return &Backend{
		chainClients:    sync.Map{},
...
func convertNetwork(network *lightz.Network) bdk.Network {
	switch network {
	case lightz.MainNet:
...
func (backend *Backend) NewWallet(credentials *onchain.WalletCredentials) (onchain.Wallet, error) {
	...
	info.Currency = lightz.CurrencyBtc
