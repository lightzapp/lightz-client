...
import (
	...
	onchainWallet "github.com/lightzapp/lightz-client/internal/onchain/wallet"
	"github.com/lightzapp/lightz-client/internal/test"
	"github.com/lightzapp/lightz-client/pkg/lightz"
	"github.com/stretchr/testify/require"
)

var wallets map[lightz.Currency]*onchainWallet.Wallet
...
func TestWallet_BumpTransactionFee(t *testing.T) {
	...
	wallet := getWallet(lightz.CurrencyBtc)
...
func TestBalance(t *testing.T) {
	balance, err := getWallet(lightz.CurrencyBtc).GetBalance()
	require.NoError(t, err)
	require.NotZero(t, balance.Total)
}

func getWallet(currency lightz.Currency) *onchainWallet.Wallet {
	...
	for _, currency := range []lightz.Currency{lightz.CurrencyBtc, lightz.CurrencyLiquid} {
		t.Run(string(currency), func(t *testing.T) {
			...
			t.Run("Normal", func(t *testing.T) {
				...
				tx, err := lightz.NewTxFromHex(currency, rawTx, nil)
				require.NoError(t, err)
				if btcTx, ok := tx.(*lightz.BtcTransaction); ok {
					for _, txIn := range btcTx.MsgTx().TxIn {
						require.Equalf(t, wire.MaxTxInSequenceNum-2, txIn.Sequence, "rbf should be enabled")
					}
				} else if liquidTx, ok := tx.(*lightz.LiquidTransaction); ok {
...
func TestReal(t *testing.T) {
	btcWallet := getWallet(lightz.CurrencyBtc)
