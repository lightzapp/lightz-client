...
func (wallet *Wallet) Connect() error {
	...
	case lightz.CurrencyBtc:
		...
		if electrum == nil && config.Network == lightz.Regtest {
...
func (wallet *Wallet) SetSubaccount(subaccount *uint64) (*uint64, error) {
	...
	if subaccount == nil {
		...
		if subaccount == nil {
			details, free := toJson(map[string]any{"name": "lightzd", "type": accType})
			defer free()
			var result struct {
				Pointer uint64 `json:"pointer"`
			}
...
func Login(credentials *onchain.WalletCredentials) (*Wallet, error) {
	...
	if credentials.Mnemonic != "" {
		...
		if credentials.Currency == lightz.CurrencyLiquid {
...
func (wallet *Wallet) createTransaction(args onchain.WalletSendArgs) (map[string]any, error) {
	...
	if wallet.Currency == lightz.CurrencyLiquid {
