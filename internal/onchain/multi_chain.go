...
// potentially using a lightzd chain provider as a fallback.
// Its a trust-minimized implementation where lightzd API is only ever trusted if every single provider
// is not reachable.
...
func (m MultiChainProvider) allProviders() []ChainProvider {
	...
	return append(m.Providers, m.Lightz)
}
...
func (m MultiChainProvider) BroadcastTransaction(txHex string) (txId string, err error) {
	...
	// Broadcasting transactions via all known (including lightzd) providers is fine
...
func (m MultiChainProvider) IsTransactionConfirmed(txId string) (confirmed bool, err error) {
	var result bool
	var merr multierror.Error
	// It's important to not include lightzd by default here, and first try all providers
	...
	if len(merr.Errors) == len(m.Providers) {
		if m.Lightz != nil {
			logger.Warnf("All providers failed to check if transaction (%s) is confirmed, falling back to lightzd: %v", txId, &merr)
			return m.Lightz.IsTransactionConfirmed(txId)
		}
...
func (m MultiChainProvider) GetBlockHeight() (uint32, error) {
	...
	// lightzd by default since it could cause issues with the IsTransactionConfirmed call
	// where the other providers aren't yet synced up to the height lightzd returned.
	for _, provider := range m.Providers {
	...
	if len(merr.Errors) == len(m.Providers) {
		if m.Lightz != nil {
			logger.Warnf("All providers failed to get block height, falling back to lightzd: %v", &merr)
			return m.Lightz.GetBlockHeight()
		}
		return result, fmt.Errorf("all providers failed: %v", &merr)
	}
	return result, nil
