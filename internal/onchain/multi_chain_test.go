...
func TestMultiChainProvider_IsTransactionConfirmed(t *testing.T) {
	...
	tests := []struct {
		name          string
		providers     func(t *testing.T) []onchain.ChainProvider
		lightzd       func(t *testing.T) onchain.ChainProvider
		wantConfirmed bool
		wantErr       bool
	}{
		{
		...
		{
			name: "all provider failure -> lightzd fallback",
			providers: func(t *testing.T) []onchain.ChainProvider {
				return []onchain.ChainProvider{errorProvider(t), errorProvider(t)}
			},
			lightzd: func(t *testing.T) onchain.ChainProvider {
...
func TestMultiChainProvider_GetBlockHeight(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "all provider failure -> lightzd failure",
			providers: func(t *testing.T) []onchain.ChainProvider {
				return []onchain.ChainProvider{errorProvider(t), errorProvider(t)}
			},
			lightzd: func(t *testing.T) onchain.ChainProvider {
		...
		{
			name: "all provider failure -> no lightzd",
			providers: func(t *testing.T) []onchain.ChainProvider {
				return []onchain.ChainProvider{errorProvider(t), errorProvider(t)}
			},
			lightzd: nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var lightzProvider onchain.ChainProvider
			if tt.lightzd != nil {
				lightzProvider = tt.lightzd(t)
			}

			provider := onchain.MultiChainProvider{
				Providers: tt.providers(t),
				Lightz:    lightzProvider,
			}
