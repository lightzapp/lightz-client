...
func walletTest(t *testing.T, funded bool, f func(t *testing.T, wallet onchain.Wallet)) {
	test.InitLogger()
	for _, currency := range []lightz.Currency{lightz.CurrencyLiquid, lightz.CurrencyBtc} {
			...
			require.Eventually(t, func() bool {
				...
				for _, tx := range transactions {
					if tx.Id == txId {
						...
						if walletInfo.Currency == lightz.CurrencyLiquid {
...
func TestWallet_BumpTransactionFee(t *testing.T) {
	...
	t.Run("BTC", func(t *testing.T) {
		backend := test.WalletBackend(t, lightz.CurrencyBtc)
		wallet, err := backend.NewWallet(test.WalletCredentials(lightz.CurrencyBtc))
		require.NoError(t, err)
		require.NoError(t, test.FundWallet(lightz.CurrencyBtc, wallet))

		cli := test.GetCli(lightz.CurrencyBtc)
		toAddress := test.GetNewAddress(cli)
