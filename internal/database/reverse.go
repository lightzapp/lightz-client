...
func (reverseSwap *ReverseSwap) Serialize() ReverseSwapSerialized {
	return ReverseSwapSerialized{
		...
		State:               lightzrpc.SwapState_name[int32(reverseSwap.State)],
...
func parseReverseSwap(rows *sql.Rows) (*ReverseSwap, error) {
	...
	reverseSwap.Status = lightz.ParseEvent(status)
...
func (database *Database) QueryClaimableReverseSwaps(tenantId *Id, currency lightz.Currency) ([]*ReverseSwap, error) {
	...
	values := []any{currency, lightz.TransactionRefunded.String()}
	if tenantId != nil {
		query += " AND tenantId = ?"
