...
func (database *Database) queryChainSwapData(id string, currency lightz.Currency, isClaim bool) (data *ChainSwapData, err error) {
...
func (database *Database) QueryRefundableChainSwaps(tenantId *Id, currency lightz.Currency, currentBlockHeight uint32) ([]*ChainSwap, error) {
	...
	values := []any{currency, lightz.TransactionLockupFailed.String(), lightz.TransactionFailed.String(), lightzrpc.SwapState_SUCCESSFUL, currentBlockHeight}
	if tenantId != nil {
		query += " AND tenantId = ?"
...
func (database *Database) QueryClaimableChainSwaps(tenantId *Id, currency lightz.Currency) ([]*ChainSwap, error) {
	...
	values := []any{currency, lightzrpc.SwapState_REFUNDED}
	if tenantId != nil {
		query += " AND tenantId = ?"
		values = append(values, tenantId)
	}
...
func (database *Database) parseChainSwap(rows *sql.Rows) (*ChainSwap, error) {
	...
	swap.Status = lightz.ParseEvent(status)
...
func parseChainSwapData(rows *sql.Rows, isClaim bool) (*ChainSwapData, error) {
	...
	swapTree := JsonScanner[*lightz.SerializedTree]{Nullable: true}
