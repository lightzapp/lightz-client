...
func (swap *Swap) Serialize() SwapSerialized {
	...
	return SwapSerialized{
		...
		State:               lightzrpc.SwapState_name[int32(swap.State)],
		Error:               swap.Error,
...
func (database *Database) QueryRefundableSwaps(tenantId *Id, currency lightz.Currency, currentBlockHeight uint32) ([]*Swap, error) {
	...
	values := []any{currency, lightzrpc.SwapState_SERVER_ERROR, lightzrpc.SwapState_ERROR, lightzrpc.SwapState_SUCCESSFUL, currentBlockHeight}
	if tenantId != nil {
		query += " AND tenantId = ?"
...
func (database *Database) SetSwapRefundTransactionId(swap *Swap, refundTransactionId string, fee uint64) error {
	swap.State = lightzrpc.SwapState_REFUNDED
