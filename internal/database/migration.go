...
func (database *Database) performMigration(tx *Transaction, oldVersion int) error {
	...
	case 1:
		...
		for _, swapToUpdate := range swapsToUpdate {
			status := lightz.ParseEvent(swapToUpdate.status)

			var newState lightzrpc.SwapState

			if status.IsCompletedStatus() {
				newState = lightzrpc.SwapState_SUCCESSFUL
			} else if status.IsFailedStatus() {
				newState = lightzrpc.SwapState_SERVER_ERROR
			} else {
				// Handle deprecated events
				switch swapToUpdate.status {
				case "swap.refunded":
					newState = lightzrpc.SwapState_REFUNDED

				case "swap.abandoned":
					newState = lightzrpc.SwapState_ABANDONED

				default:
					newState = lightzrpc.SwapState_PENDING
				}
			}
		...
		for _, reverseSwapToUpdate := range reverseSwapsToUpdate {
			status := lightz.ParseEvent(reverseSwapToUpdate.status)
	...
	case 3:
		...
		rows, err := tx.Query("SELECT id FROM swaps WHERE state = ?", lightzrpc.SwapState_PENDING)
		if err != nil {
			return err
		}
		if rows.Next() {
		...
		rows, err = tx.Query("SELECT id FROM reverseSwaps WHERE state = ?", lightzrpc.SwapState_PENDING)
		if err != nil {
			return err
		}
		if rows.Next() {
...
`
		...
		for _, reverseSwap := range reverseSwaps {
			decoded, err := zpay32.Decode(reverseSwap.Invoice, lightz.MainNet.Btc)
			if err != nil {
				decoded, err = zpay32.Decode(reverseSwap.Invoice, lightz.TestNet.Btc)
				if err != nil {
					decoded, err = zpay32.Decode(reverseSwap.Invoice, lightz.Regtest.Btc)
				}
			}
			if err == nil && decoded.MilliSat != nil {
				if _, err := tx.Exec("UPDATE reverseSwaps SET invoiceAmount = ? WHERE id = ?", decoded.MilliSat.ToLokis(), reverseSwap.Id); err != nil {
					return err
				}
			}
		}
...
`
		...
		for _, swap := range swaps {
			if swap.Invoice != "" {
				decoded, err := lightning.DecodeInvoice(swap.Invoice, lightz.MainNet.Btc)
				if err != nil {
					decoded, err = lightning.DecodeInvoice(swap.Invoice, lightz.TestNet.Btc)
					if err != nil {
						decoded, err = lightning.DecodeInvoice(swap.Invoice, lightz.Regtest.Btc)
					}
				}
				if err == nil {
