...
func New(
	...
	nursery := &Nursery{
		...
		lightzWs:         lightzclient.NewWebsocket(),
...
func (nursery *Nursery) sendUpdate(id string, update SwapUpdate) {
	if update.IsFinal {
		nursery.lightzWs.Unsubscribe(id)
	}
...
func (nursery *Nursery) Init() error {
	...
	if nursery.maxZeroConfAmount == 0 {
		pairs, err := nursery.lightz.GetSubmarinePairs()
		if err != nil {
			return fmt.Errorf("could not get submarine pairs: %v", err)
		}
		pair, err := lightz.FindPair(lightz.Pair{From: lightz.CurrencyLiquid, To: lightz.CurrencyBtc}, pairs)
		if err != nil {
			return fmt.Errorf("could not find submarine pair: %v", err)
		}
		nursery.maxZeroConfAmount = pair.Limits.MaximalZeroConfAmount
		logger.Infof("No maximal zero conf amount set, using same value as lightzd: %v", nursery.maxZeroConfAmount)
	}

	if err := nursery.lightzWs.Connect(); err != nil {
		return fmt.Errorf("could not connect to lightzd websocket: %v", err)
	}

	nursery.BtcBlocks = nursery.startBlockListener(lightz.CurrencyBtc)
	nursery.LiquidBlocks = nursery.startBlockListener(lightz.CurrencyLiquid)
...
func (nursery *Nursery) Stop() {
	nursery.cancel()
	if err := nursery.lightzWs.Close(); err != nil {
		logger.Errorf("Could not close lightzd websocket: %v", err)
	}
...
func (nursery *Nursery) registerSwaps(swapIds []string) error {
	...
	if err := nursery.lightzWs.Subscribe(swapIds); err != nil {
		return err
	}
...
func (nursery *Nursery) processUpdate(status lightz.SwapUpdate) error {
	...
	if status.Error != "" {
		return fmt.Errorf("lightzd could not find Swap %s: %s", status.Id, status.Error)
	}
	if swap != nil {
...
func (nursery *Nursery) startSwapListener() {
	...
	go func() {
		for status := range nursery.lightzWs.Updates {
...
func (nursery *Nursery) createTransaction(currency lightz.Currency, outputs []*Output) (id string, err error) {
	...
	transaction, results, err := lightz.ConstructTransaction(nursery.network, currency, details, lightz.Fee{SatsPerVbyte: &feeSatPerVbyte}, nursery.lightz)
	if err != nil {
...
func (nursery *Nursery) populateOutputs(outputs []*Output) (valid []*Output, details []lightz.OutputDetails) {
...
func (nursery *Nursery) GetFeeEstimations(swapType lightz.SwapType, pair lightz.Pair) (lightz.FeeEstimations, error) {
	fees := make(lightz.FeeEstimations)
	var err error
	for _, currency := range lightz.RequiredEstimations(swapType, pair) {
...
func (nursery *Nursery) CheckAmounts(swapType lightz.SwapType, pair lightz.Pair, sendAmount uint64, receiveAmount uint64, serviceFee lightz.Percentage) (err error) {
	...
	return lightz.CheckAmounts(swapType, pair, sendAmount, receiveAmount, serviceFee, fees, false)
}
