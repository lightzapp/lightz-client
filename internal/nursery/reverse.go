...
func (nursery *Nursery) sendReverseSwapUpdate(reverseSwap database.ReverseSwap) {
	nursery.sendUpdate(reverseSwap.Id, SwapUpdate{
		...
		IsFinal: reverseSwap.State == lightzrpc.SwapState_SUCCESSFUL || reverseSwap.State == lightzrpc.SwapState_SERVER_ERROR,
	})
}
	...
	go func() {
		...
		if err != nil {
			if nursery.ctx.Err() == nil {
				...
				if dbErr := nursery.database.UpdateReverseSwapState(reverseSwap, lightzrpc.SwapState_ERROR, err.Error()); dbErr != nil {
...
func (nursery *Nursery) finalizeReverseSwap(reverseSwap *database.ReverseSwap) error {
	...
	serviceFee := lightz.CalculatePercentage(reverseSwap.ServiceFeePercent, invoiceAmount)
	onchainFee := invoiceAmount - int64(reverseSwap.OnchainAmount) - serviceFee
	if onchainFee < 0 {
		logger.Warnf("Reverse Swap %s has negative lightzd onchain fee: %dsat", reverseSwap.Id, onchainFee)
		onchainFee = 0
	}

	logger.Infof("Reverse Swap service fee: %dsat; lightzd onchain fee: %dsat", serviceFee, onchainFee)

	if err := nursery.database.SetReverseSwapServiceFee(reverseSwap, serviceFee, uint64(onchainFee)); err != nil {
		return fmt.Errorf("could not set reverse swap service fee in database: %w", err)
	}
	if err := nursery.database.UpdateReverseSwapState(reverseSwap, lightzrpc.SwapState_SUCCESSFUL, ""); err != nil {
...
func (nursery *Nursery) handleReverseSwapStatus(reverseSwap *database.ReverseSwap, event lightz.SwapStatusResponse) {
	parsedStatus := lightz.ParseEvent(event.Status)
	...
	switch parsedStatus {
	case lightz.TransactionDirect:
		var blindingKey *btcec.PrivateKey
		var wallet onchain.Wallet
		if reverseSwap.Pair.To == lightz.CurrencyLiquid && reverseSwap.WalletId != nil {
			var err error
		...
		directTransaction, err := lightz.NewTxFromHex(reverseSwap.Pair.To, event.Transaction.Hex, blindingKey)
		if err != nil {
	...
	if parsedStatus.IsFailedStatus() {
		if reverseSwap.State == lightzrpc.SwapState_PENDING {
			if err := nursery.database.UpdateReverseSwapState(reverseSwap, lightzrpc.SwapState_SERVER_ERROR, ""); err != nil {
...
func (nursery *Nursery) chooseDirectOutput(swap *database.ReverseSwap, feeEstimations lightz.FeeEstimations, outputs []*onchain.Output) (*onchain.Output, error) {
...
func (nursery *Nursery) handleReverseSwapDirectPayments(swap *database.ReverseSwap, outputs []*onchain.Output) {
	err := nursery.database.RunTx(func(tx *database.Transaction) error {
		...
		feeEstimations, err := nursery.GetFeeEstimations(lightz.ReverseSwap, swap.Pair)
		if err != nil {
		...
		if swap.Pair.To == lightz.CurrencyBtc || output.Value > nursery.maxZeroConfAmount {
			...
			if !errors.Is(err, errors.ErrUnsupported) {
				...
				if !confirmed {
					...
					if err := tx.UpdateReverseSwapStatus(swap, lightz.TransactionDirectMempool); err != nil {
		...
		if err := tx.UpdateReverseSwapState(swap, lightzrpc.SwapState_SUCCESSFUL, ""); err != nil {
			return fmt.Errorf("could not update swap: %s", err)
		}
		if err := tx.UpdateReverseSwapStatus(swap, lightz.TransactionDirect); err != nil {
			return fmt.Errorf("set reverse swap status: %s", err)
		}
...
func (nursery *Nursery) checkExternalReverseSwaps(currency lightz.Currency) error {
	reverseSwaps, err := nursery.database.QueryReverseSwaps(database.SwapQuery{
		To:     &currency,
		States: []lightzrpc.SwapState{lightzrpc.SwapState_PENDING, lightzrpc.SwapState_ERROR},
	})
	if err != nil {
		return err
	}
	for _, swap := range reverseSwaps {
		...
		if err != nil {
			if errors.Is(err, errors.ErrUnsupported) && currency == lightz.CurrencyBtc {
