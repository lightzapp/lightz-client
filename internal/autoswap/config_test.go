...
func TestGetPair(t *testing.T) {
	...
	pair := cfg.GetPair(lightzrpc.SwapType_SUBMARINE)
	require.Equal(t, lightzrpc.Currency_LBTC, pair.From)
	require.Equal(t, lightzrpc.Currency_BTC, pair.To)

	pair = cfg.GetPair(lightzrpc.SwapType_REVERSE)
	require.Equal(t, lightzrpc.Currency_LBTC, pair.To)
	require.Equal(t, lightzrpc.Currency_BTC, pair.From)

	cfg.Currency = lightzrpc.Currency_BTC
...
func TestLightningConfig(t *testing.T) {
	tt := []struct {
		...
		},
		{
			...
			wallets: []onchain.WalletInfo{{Id: 1, Name: "test", Currency: lightz.CurrencyBtc}},
			err:     false,
		},
		{
		...
		},
		{
			...
			wallets: []onchain.WalletInfo{{Id: 1, Name: "test", Currency: lightz.CurrencyLiquid}},
			err:     true,
		},
		{
		...
		},
		{
			...
			wallets: []onchain.WalletInfo{{Id: 1, Name: "test", Currency: lightz.CurrencyLiquid, Readonly: true}},
			err:     false,
		},
		{
...
func TestChainConfig(t *testing.T) {
	btcWallet := onchain.WalletInfo{Id: 1, Name: "btc", Currency: lightz.CurrencyBtc}
	liquidWallet := onchain.WalletInfo{Id: 2, Name: "liquid", Currency: lightz.CurrencyLiquid}
