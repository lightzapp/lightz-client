...
func (cfg *ChainConfig) Init() (err error) {
	...
	if cfg.ToAddress != "" {
		cfg.pair.To, err = lightz.GetAddressCurrency(cfg.onchain.Network, cfg.ToAddress)
		if err != nil {
...
func (cfg *ChainConfig) getRecommendation() (*autoswaprpc.ChainRecommendation, error) {
	...
	pairInfo, err := cfg.rpc.GetAutoSwapPairInfo(lightzrpc.SwapType_CHAIN, serializers.SerializePair(cfg.pair))
	if err != nil {
	...
	if balance.Confirmed > cfg.MaxBalance {
		...
		sendFee, err := cfg.rpc.WalletSendFee(&lightzrpc.WalletSendRequest{SendAll: &sendAll, Amount: amount, Id: cfg.fromWallet.GetWalletInfo().Id})
		if err != nil {
		...
		pendingSwaps, err := cfg.database.QueryChainSwaps(database.SwapQuery{
			States:   []lightzrpc.SwapState{lightzrpc.SwapState_PENDING},
			TenantId: &cfg.tenant.Id,
		})
		if err != nil {
