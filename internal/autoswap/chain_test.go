...
func newPairInfo() *lightzrpc.PairInfo {
...
func TestChainSwapper(t *testing.T) {
	walletInfo := onchain.WalletInfo{Id: 1, Name: "test", Currency: lightz.CurrencyLiquid}
	...
	setup := func(t *testing.T) (*AutoSwap, *ChainSwapper, *MockRpcProvider, *onchainmock.MockWallet) {
		fromWallet := mockedWallet{info: onchain.WalletInfo{Id: 1, Name: "test", Currency: lightz.CurrencyLiquid}}.Create(t)
	...
	t.Run("GetRecommendation", func(t *testing.T) {
		...
		tests := []struct {
			...
			},
			{
				...
				setup: func(t *testing.T, shared shared) {
					mockRpc(shared).EXPECT().GetAutoSwapPairInfo(mock.Anything, mock.Anything).Return(&lightzrpc.PairInfo{
						Limits: &lightzrpc.Limits{
							Minimal: 100000000,
							Maximal: 1000000000,
						},
			...
			},
			{
				...
				expectedAmount: 5000,
				setup: func(t *testing.T, shared shared) {
					mockRpc(shared).EXPECT().WalletSendFee(mock.Anything).RunAndReturn(func(request *lightzrpc.WalletSendRequest) (*lightzrpc.WalletSendFee, error) {
						require.True(t, request.GetSendAll())
						return &lightzrpc.WalletSendFee{Amount: 5000}, nil
					})
				},
			},
			{
		...
		for _, tc := range tests {
			t.Run(tc.name, func(t *testing.T) {
				...
				mockRpc(shared).EXPECT().GetAutoSwapPairInfo(lightzrpc.SwapType_CHAIN, mock.Anything).Return(newPairInfo(), nil)
				mockRpc(shared).EXPECT().WalletSendFee(mock.Anything).RunAndReturn(func(request *lightzrpc.WalletSendRequest) (*lightzrpc.WalletSendFee, error) {
					return &lightzrpc.WalletSendFee{Amount: request.Amount}, nil
				}).Maybe()
	...
	t.Run("Execute", func(t *testing.T) {
		...
		rpcMock.EXPECT().CreateAutoChainSwap(mock.Anything, mock.Anything).RunAndReturn(func(tenant *database.Tenant, request *lightzrpc.CreateChainSwapRequest) error {
	...
	t.Run("Start", func(t *testing.T) {
		...
		rpcMock.EXPECT().GetAutoSwapPairInfo(lightzrpc.SwapType_CHAIN, mock.Anything).Return(pairInfo, nil).Once()
		rpcMock.EXPECT().CreateAutoChainSwap(mock.Anything, mock.Anything).Return(nil).Once()
		rpcMock.EXPECT().WalletSendFee(mock.Anything).RunAndReturn(func(request *lightzrpc.WalletSendRequest) (*lightzrpc.WalletSendFee, error) {
			return &lightzrpc.WalletSendFee{Amount: request.Amount}, nil
		}).Maybe()
