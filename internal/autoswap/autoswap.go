...
type RpcProvider interface {
	GetAutoSwapPairInfo(swapType lightzrpc.SwapType, pair *lightzrpc.Pair) (*lightzrpc.PairInfo, error)
	GetLightningChannels() ([]*lightning.LightningChannel, error)
	GetBlockUpdates(currency lightz.Currency) (<-chan *onchain.BlockEpoch, func())
	WalletSendFee(request *lightzrpc.WalletSendRequest) (*lightzrpc.WalletSendFee, error)

	CreateAutoSwap(tenant *database.Tenant, request *lightzrpc.CreateSwapRequest) error
	CreateAutoReverseSwap(tenant *database.Tenant, request *lightzrpc.CreateReverseSwapRequest) error
	CreateAutoChainSwap(tenant *database.Tenant, request *lightzrpc.CreateChainSwapRequest) error
}
