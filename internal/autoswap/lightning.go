...
const DefaultReserve = lightz.Percentage(5)
...
func (cfg *LightningConfig) Init() error {
	var err error
	cfg.swapType, err = lightz.ParseSwapType(cfg.SwapType)
	if err != nil {
		return fmt.Errorf("invalid swap type: %w", err)
	}

	cfg.currency = serializers.ParseCurrency(&cfg.Currency)
	cfg.maxFeePercent = lightz.Percentage(cfg.MaxFeePercent)
	...
	if cfg.InboundBalance == 0 && cfg.OutboundBalance == 0 {
		cfg.outboundBalance.Relative = lightz.Percentage(cfg.OutboundBalancePercent)
		cfg.inboundBalance.Relative = lightz.Percentage(cfg.InboundBalancePercent)
		maxPercent := lightz.Percentage(100) - 2*cfg.reserve
...
func (cfg *LightningConfig) InitWallet() (err error) {
	...
	if cfg.Wallet != "" {
		...
		if err = lightz.ValidateAddress(cfg.onchain.Network, cfg.StaticAddress, cfg.currency); err != nil {
...
func (cfg *LightningConfig) GetPair(swapType lightzrpc.SwapType) *lightzrpc.Pair {
	currency := cfg.Currency
	result := &lightzrpc.Pair{}
	switch swapType {
	case lightzrpc.SwapType_SUBMARINE:
		result.From = currency
		result.To = lightzrpc.Currency_BTC
	case lightzrpc.SwapType_REVERSE:
		result.From = lightzrpc.Currency_BTC
		result.To = currency
	}
	return result
}

func (cfg *LightningConfig) Allowed(swapType lightz.SwapType) bool {
	return cfg.swapType == swapType || cfg.swapType == ""
}

func (cfg *LightningConfig) AllowNormalSwaps() bool {
	return cfg.Allowed(lightz.NormalSwap)
}

func (cfg *LightningConfig) AllowReverseSwaps() bool {
	return cfg.Allowed(lightz.ReverseSwap)
}
...
func (cfg *LightningConfig) getDismissedChannels() (DismissedChannels, error) {
	...
	query.Include = lightzrpc.IncludeSwaps_AUTO
...
func (cfg *LightningConfig) validateRecommendations(
	...
	for _, recommendation := range recommendations {
		...
		if swap != nil {
			...
			if balance != nil && swap.Type == lightz.NormalSwap && swap.Amount > balance.Confirmed {
...
func (cfg *LightningConfig) execute(recommendation *autoswaprpc.LightningRecommendation, force bool) error {
	...
	switch swap.Type {
	case lightzrpc.SwapType_REVERSE:
		err = cfg.rpc.CreateAutoReverseSwap(&database.DefaultTenant, &lightzrpc.CreateReverseSwapRequest{
			Amount:         swap.Amount,
	...
	case lightzrpc.SwapType_SUBMARINE:
		err = cfg.rpc.CreateAutoSwap(&database.DefaultTenant, &lightzrpc.CreateSwapRequest{
			Amount: swap.Amount,
			Pair:   pair,
			//ChanIds:          chanIds,
