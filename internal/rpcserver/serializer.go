...
func serializeSwap(swap *database.Swap) *lightzrpc.SwapInfo {
	if swap == nil {
		return nil
	}
	serializedSwap := swap.Serialize()

	serialized := &lightzrpc.SwapInfo{
...
func serializeAnySwap(swap *database.AnySwap) *lightzrpc.AnySwapInfo {
...
func serializeReverseSwap(reverseSwap *database.ReverseSwap) *lightzrpc.ReverseSwapInfo {
	if reverseSwap == nil {
		return nil
	}
	serializedReverseSwap := reverseSwap.Serialize()

	return &lightzrpc.ReverseSwapInfo{
...
func serializeChainSwap(chainSwap *database.ChainSwap) *lightzrpc.ChainSwapInfo {
	if chainSwap == nil {
		return nil
	}
	serializedChainSwap := chainSwap.Serialize()

	return &lightzrpc.ChainSwapInfo{
...
func serializeChainSwapData(chainSwap *database.ChainSwapData) *lightzrpc.ChainSwapData {
	if chainSwap == nil {
		return nil
	}
	serializedChainSwap := chainSwap.Serialize()

	return &lightzrpc.ChainSwapData{
		Id:                  serializedChainSwap.Id,
...
func serializeReversePair(pair lightz.Pair, reversePair *lightz.ReversePair) *lightzrpc.PairInfo {
...
func serializeChainPair(pair lightz.Pair, chainPair *lightz.ChainPair) *lightzrpc.PairInfo {
...
//nolint:staticcheck
func serializeWalletSubaccount(subaccount wallet.Subaccount, balance *onchain.Balance) *lightzrpc.Subaccount {
	//nolint:staticcheck
	return &lightzrpc.Subaccount{
...
func serializeWalletCredentials(credentials *onchain.WalletCredentials) *lightzrpc.WalletCredentials {
	return &lightzrpc.WalletCredentials{
...
func serializeTenant(tenant *database.Tenant) *lightzrpc.Tenant {
	if tenant == nil {
		return nil
	}
	return &lightzrpc.Tenant{
		Id:   tenant.Id,
		Name: tenant.Name,
	}
}
