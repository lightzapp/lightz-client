...
import (
	...
	"github.com/lightzapp/lightz-client/internal/electrum"
	"github.com/lightzapp/lightz-client/internal/esplora"
	"github.com/lightzapp/lightz-client/internal/mempool"
...
type RpcServer struct {
	cfg            *config.Config
	grpc           *grpc.Server
	lightzServer   *routedLightzServer
...
func (server *RpcServer) Init() error {
	...
	server.lightzServer = &routedLightzServer{
		database:       server.cfg.Database,
		stop:           make(chan bool),
		state:          stateLightningSyncing,
		swapper:        swapper,
		referralId:     server.cfg.ReferralId,
		walletBackends: make(map[lightz.Currency]onchain.WalletBackend),
	}
	server.autoswapServer = &routedAutoSwapServer{
		database: server.cfg.Database,
		swapper:  swapper,
	}
	unaryInterceptors := []grpc.UnaryServerInterceptor{server.lightzServer.UnaryServerInterceptor()}
	streamInterceptors := []grpc.StreamServerInterceptor{server.lightzServer.StreamServerInterceptor()}
	...
	if rpcCfg.Password != "" {
		...
		server.lightzServer.macaroon = macaroon
	} else {
	...
	lightzrpc.RegisterLightzServer(server.grpc, server.lightzServer)
	autoswaprpc.RegisterAutoSwapServer(server.grpc, server.autoswapServer)

	return nil
}

func (server *routedLightzServer) initLightning(cfg *config.Config) error {
...
func (server *routedLightzServer) start(cfg *config.Config) (err error) {
	...
	server.network, err = lightz.ParseChain(cfg.Network)
	if err != nil {
		return err
	}

	if server.network == lightz.TestNet {
		return errors.New("testnet is deprecated, use regtest instead")
	}

	if server.lightz == nil {
		server.lightz, err = initLightz(cfg, server.network)
		if err != nil {
			return fmt.Errorf("could not init Lightz API: %w", err)
		}
	}

	if server.onchain == nil {
		server.onchain, err = initOnchain(cfg, server.lightz, server.network)
		if err != nil {
	...
	server.walletBackends[lightz.CurrencyLiquid], err = liquid_wallet.NewBackend(liquidConfig)
	if err != nil {
	...
	server.walletBackends[lightz.CurrencyBtc], err = bitcoin_wallet.NewBackend(bitcoinConfig)
	if err != nil {
...
func (server *RpcServer) Start() chan error {
	go func() {
		if err := server.lightzServer.start(server.cfg); err != nil {
			logger.Fatal(fmt.Sprintf("Could not start Lightz server: %v", err))
		}
	}()
	...
	if !cfg.RestDisabled {
		...
		go func() {
			...
			err = lightzrpc.RegisterLightzHandlerFromEndpoint(
				context.Background(),
				mux,
				sanitizedRpcUrl,
...
func (server *RpcServer) Stop() error {
	_, err := server.lightzServer.Stop(context.Background(), nil)
	return err
}

func initLightz(cfg *config.Config, network *lightz.Network) (*lightz.Api, error) {
	lightzUrl := cfg.Lightz.URL
	if lightzUrl == "" {
		lightzUrl = network.DefaultLightzUrl
		logger.Infof("Using default Lightz endpoint for network %s: %s", network.Name, lightzUrl)
	} else {
		logger.Info("Using configured Lightz endpoint: " + lightzUrl)
	}

	lightzApi := &lightz.Api{URL: lightzUrl, Referral: cfg.ReferralId}
	if cfg.Proxy != "" {
		...
		lightzApi.Client = http.Client{
			Transport: transport,
			Timeout:   30 * time.Second,
		}
	}

	return lightzApi, nil
}

func defaultChainProvider(cfg *config.Config, lightzApi *lightz.Api, currency lightz.Currency, network *lightz.Network) onchain.ChainProvider {
	...
	return onchain.MultiChainProvider{
		Providers: clients,
		Lightz:    onchain.NewLightzChainProvider(lightzApi, currency),
	}
}

func initOnchain(cfg *config.Config, lightzApi *lightz.Api, network *lightz.Network) (*onchain.Onchain, error) {
	chain := &onchain.Onchain{
		Btc: &onchain.Currency{
			Chain: defaultChainProvider(cfg, lightzApi, lightz.CurrencyBtc, network),
		},
		Liquid: &onchain.Currency{
			Chain: defaultChainProvider(cfg, lightzApi, lightz.CurrencyLiquid, network),
		},
		Network: network,
	}
