...
func loadConfig(t *testing.T) *config.Config {
	...
	cfg.Database.Path = cfg.DataDir + "/lightz.db"
	return cfg
}

func getLightz(t *testing.T, cfg *config.Config) *lightz.Api {
	lightzApi, err := initLightz(cfg, lightz.Regtest)
	require.NoError(t, err)
	return lightzApi
}
...
func lessValueChainProvider(t *testing.T, original onchain.ChainProvider) *onchainmock.MockChainProvider {
	...
	txMock.EXPECT().GetRawTransaction(mock.Anything).RunAndReturn(func(txId string) (string, error) {
		...
		require.NoError(t, err)
		transaction, err := lightz.NewBtcTxFromHex(raw)
		require.NoError(t, err)
...
func getOnchain(t *testing.T, cfg *config.Config) *onchain.Onchain {
	lightzApi := getLightz(t, cfg)
	chain, err := initOnchain(cfg, lightzApi, lightz.Regtest)
	require.NoError(t, err)
	return chain
}

func getTransactionFee(t *testing.T, chain *onchain.Onchain, currency lightz.Currency, txId string) uint64 {
...
func walletName(currency lightzrpc.Currency) string {
...
func waitForSync(t *testing.T, client client.Lightz) {
...
func setup(t *testing.T, options setupOptions) (client.Lightz, client.AutoSwap, func()) {
	cfg := options.cfg
	if cfg == nil {
		cfg = loadConfig(t)
	}
	...
	rpc.lightzServer.lightz = options.lightzApi
	rpc.lightzServer.onchain = options.chain
	rpc.lightzServer.lightning = options.lightning
	go func() {
		err := rpc.lightzServer.start(cfg)
		if err != nil {
			logger.Warn("error starting lightzd server: " + err.Error())
		} else {
			for _, wallet := range options.wallets {
				rpc.lightzServer.onchain.AddWallet(wallet)
			}
		}
	}()
	...
	lightzclient := client.NewLightzClient(clientConn)
	autoSwapClient := client.NewAutoSwapClient(clientConn)

	if !options.dontSync {

		waitForSync(t, lightzclient)
	}

	return lightzclient, autoSwapClient, func() {
		_, err = autoSwapClient.ResetConfig(client.LnAutoSwap)
		_, err = autoSwapClient.ResetConfig(client.ChainAutoSwap)
		require.NoError(t, lightzclient.Stop())
	}
}

func getCli(currency lightzrpc.Currency) test.Cli {
	return test.GetCli(parseCurrency(currency))
}

type streamFunc func(state lightzrpc.SwapState) *lightzrpc.GetSwapInfoResponse

type streamStatusFunc func(state lightzrpc.SwapState, status lightz.SwapUpdateEvent) *lightzrpc.GetSwapInfoResponse

func swapStream(t *testing.T, client client.Lightz, swapId string) (streamFunc, streamStatusFunc) {
	stream, err := client.GetSwapInfoStream(swapId)
	require.NoError(t, err)

	updates := make(chan *lightzrpc.GetSwapInfoResponse, 3)
	...
	streamFunc := func(state lightzrpc.SwapState, status lightz.SwapUpdateEvent) *lightzrpc.GetSwapInfoResponse {
		for {
			select {
			case update, ok := <-updates:
				if ok {
					...
					if status != lightz.SwapCreated && status.String() != currentStatus {
						continue
					}
	...
	return func(state lightzrpc.SwapState) *lightzrpc.GetSwapInfoResponse {
		return streamFunc(state, lightz.SwapCreated)
	}, streamFunc
}
...
func createTenant(t *testing.T, admin client.Lightz, name string) (*lightzrpc.Tenant, client.Connection, client.Connection) {
	tenantInfo, err := admin.CreateTenant(name)
	require.NoError(t, err)
	require.NotZero(t, tenantInfo.Id)

	write, err := admin.BakeMacaroon(&lightzrpc.BakeMacaroonRequest{
		TenantId:    &tenantInfo.Id,
		Permissions: client.FullPermissions,
	})
	require.NoError(t, err)

	read, err := admin.BakeMacaroon(&lightzrpc.BakeMacaroonRequest{
		TenantId:    &tenantInfo.Id,
...
func TestMacaroons(t *testing.T) {
	...
	tenant := client.NewLightzClient(write)
	tenantAuto := client.NewAutoSwapClient(write)
	readTenant := client.NewLightzClient(read)
	...
	t.Run("SetTenant", func(t *testing.T) {
		admin := client.NewLightzClient(conn)
	...
	t.Run("Bake", func(t *testing.T) {
		response, err := admin.BakeMacaroon(&lightzrpc.BakeMacaroonRequest{
			Permissions: client.FullPermissions,
		})
		require.NoError(t, err)

		anotherAdmin := client.NewLightzClient(conn)
		...
		_, err = anotherAdmin.BakeMacaroon(&lightzrpc.BakeMacaroonRequest{
			Permissions: client.ReadPermissions,
		})
		require.Error(t, err)
	...
	t.Run("Admin", func(t *testing.T) {
		_, err = tenant.BakeMacaroon(&lightzrpc.BakeMacaroonRequest{
			TenantId:    &tenantInfo.Id,
			Permissions: client.ReadPermissions,
		})
		require.Error(t, err)
	...
	t.Run("Wallet", func(t *testing.T) {
		testWallet := fundedWallet(t, admin, lightzrpc.Currency_LBTC)
		hasWallets := func(t *testing.T, client client.Lightz, amount int) {
		...
		walletParams := &lightzrpc.WalletParams{Currency: lightzrpc.Currency_LBTC, Name: "test"}
	...
	t.Run("Swaps", func(t *testing.T) {
		hasSwaps := func(t *testing.T, client client.Lightz, length int) {
			swaps, err := client.ListSwaps(&lightzrpc.ListSwapsRequest{})
			require.NoError(t, err)
		...
		t.Run("Admin", func(t *testing.T) {
			hasSwaps(t, admin, 0)
			_, err = admin.CreateSwap(&lightzrpc.CreateSwapRequest{})
			require.NoError(t, err)
			externalPay := false
			_, err = admin.CreateReverseSwap(&lightzrpc.CreateReverseSwapRequest{Amount: 100000, ExternalPay: &externalPay})
			require.NoError(t, err)
			hasSwaps(t, admin, 1)
			hasSwaps(t, tenant, 0)
		})
		...
		t.Run("Tenant", func(t *testing.T) {
			hasSwaps(t, tenant, 0)
			_, err = tenant.CreateSwap(&lightzrpc.CreateSwapRequest{})
			require.ErrorContains(t, err, "invoice is required in standalone mode")
			externalPay := false
			_, err = tenant.CreateReverseSwap(&lightzrpc.CreateReverseSwapRequest{Amount: 100000, ExternalPay: &externalPay})
			require.ErrorContains(t, err, "can not create reverse swap without external pay in standalone mode")
			hasSwaps(t, tenant, 0)
			externalPay = true
			_, err = tenant.CreateReverseSwap(&lightzrpc.CreateReverseSwapRequest{Amount: 100000, ExternalPay: &externalPay, Address: test.BtcCli("getnewaddress")})
			require.NoError(t, err)
			swaps, err := tenant.ListSwaps(&lightzrpc.ListSwapsRequest{})
			require.NoError(t, err)
			require.Len(t, swaps.ReverseSwaps, 1)
		})

		t.Run("Read", func(t *testing.T) {
			_, err = readTenant.CreateSwap(&lightzrpc.CreateSwapRequest{})
			requireCode(t, err, codes.PermissionDenied)
			_, err = readTenant.CreateReverseSwap(&lightzrpc.CreateReverseSwapRequest{Amount: 100000})
			requireCode(t, err, codes.PermissionDenied)
			_, err = readTenant.CreateChainSwap(&lightzrpc.CreateChainSwapRequest{})
			requireCode(t, err, codes.PermissionDenied)
		})
	})
}
...
func TestSwapInfo(t *testing.T) {
	...
	swap, err := admin.CreateSwap(&lightzrpc.CreateSwapRequest{})
	require.NoError(t, err)
	info := stream(lightzrpc.SwapState_PENDING)
	require.Equal(t, swap.Id, info.Swap.Id)

	check(t, swap.Id)

	reverseSwap, err := admin.CreateReverseSwap(&lightzrpc.CreateReverseSwapRequest{Amount: 100000, ExternalPay: &externalPay})
	require.NoError(t, err)
	info = stream(lightzrpc.SwapState_PENDING)
	...
	toWallet := fundedWallet(t, admin, lightzrpc.Currency_LBTC)
	chainSwap, err := admin.CreateChainSwap(&lightzrpc.CreateChainSwapRequest{
		Amount:      &swapAmount,
		Pair:        &lightzrpc.Pair{From: lightzrpc.Currency_BTC, To: lightzrpc.Currency_LBTC},
	...
	t.Run("List", func(t *testing.T) {
		unify := true
		request := &lightzrpc.ListSwapsRequest{Unify: &unify}
		...
		require.Equal(t, lightzrpc.SwapType_CHAIN, swaps.AllSwaps[0].Type)
		...
		require.NoError(t, err)
		require.Len(t, swaps.AllSwaps, int(limit))
		require.Equal(t, lightzrpc.SwapType_REVERSE, swaps.AllSwaps[0].Type)
		require.Equal(t, lightzrpc.SwapType_SUBMARINE, swaps.AllSwaps[1].Type)
	...
	t.Run("PaymentHash", func(t *testing.T) {
		...
		t.Run("WithInvoice", func(t *testing.T) {
			...
			swap, err = admin.CreateSwap(&lightzrpc.CreateSwapRequest{Invoice: &invoice.PaymentRequest})
			require.NoError(t, err)
...
func checkTxOutAddress(t *testing.T, chain *onchain.Onchain, currency lightz.Currency, txId string, outAddress string, cooperative bool) {
	transaction, err := chain.GetTransaction(currency, txId, nil, false)
	require.NoError(t, err)

	if tx, ok := transaction.(*lightz.BtcTransaction); ok {
		...
		if outAddress != "" {
			decoded, err := chainutil.DecodeAddress(outAddress, &chaincfg.RegressionNetParams)
			require.NoError(t, err)
	...
	} else if tx, ok := transaction.(*lightz.LiquidTransaction); ok {
...
func parseCurrency(grpcCurrency lightzrpc.Currency) lightz.Currency {
	if grpcCurrency == lightzrpc.Currency_BTC {
...
func TestReverseSwap(t *testing.T) {
	...
	checkSwap := func(t *testing.T, client client.Lightz, id string, chain *onchain.Onchain) {
	...
	tests := []struct {
		...
		{desc: "BTC/Normal", to: lightzrpc.Currency_BTC, disablePartials: true},
		{desc: "BTC/ZeroConf", to: lightzrpc.Currency_BTC, zeroConf: true, external: true},
		{desc: "BTC/Recover", to: lightzrpc.Currency_BTC, zeroConf: true, recover: true},
		{desc: "Liquid/Normal", to: lightzrpc.Currency_LBTC, disablePartials: true},
		{desc: "Liquid/ZeroConf", to: lightzrpc.Currency_LBTC, zeroConf: true, external: true},
		{desc: "Liquid/Recover", to: lightzrpc.Currency_LBTC, zeroConf: true, recover: true},
		{desc: "Wait", to: lightzrpc.Currency_BTC, zeroConf: true, waitForClaim: true},
	}
		...
		t.Run(node, func(t *testing.T) {
			for _, tc := range tests {
				t.Run(tc.desc, func(t *testing.T) {
					cfg := loadConfig(t)
					lightzApi := getLightz(t, cfg)
					lightzApi.DisablePartialSignatures = tc.disablePartials
					cfg.Node = node
					chain := getOnchain(t, cfg)
					client, _, stop := setup(t, setupOptions{cfg: cfg, lightzApi: lightzApi, chain: chain})
					...
					var info *lightzrpc.GetSwapInfoResponse
					...
					if tc.recover {
						...
						statusStream(lightzrpc.SwapState_PENDING, lightz.TransactionMempool)
						stop()
						...
						for info.GetReverseSwap().GetState() != lightzrpc.SwapState_SUCCESSFUL {
							select {
							case <-ticker.C:
						...
						if tc.waitForClaim && tc.zeroConf {
							...
							info = status(lightzrpc.SwapState_SUCCESSFUL)
						}
	...
	t.Run("Invalid", func(t *testing.T) {
		...
		for _, tc := range tests {
			t.Run(tc.desc, func(t *testing.T) {
				...
				swap, err := client.CreateReverseSwap(&lightzrpc.CreateReverseSwapRequest{
					Amount:         100000,
				...
				statusStream(lightzrpc.SwapState_PENDING, lightz.TransactionMempool)
				test.MineBlock()
				info := statusStream(lightzrpc.SwapState_ERROR, lightz.TransactionConfirmed)
	...
	t.Run("Retry", func(t *testing.T) {
		...
		statusStream(lightzrpc.SwapState_ERROR, lightz.TransactionConfirmed)
		test.MineBlock()
		// new block triggers a retry, on which the broadcast will succeed
		statusStream(lightzrpc.SwapState_SUCCESSFUL, lightz.InvoiceSettled)
		checkSwap(t, client, swap.Id, chain)
	})
	...
	t.Run("Standalone", func(t *testing.T) {
		...
		stream(lightzrpc.SwapState_PENDING)
		info := stream(lightzrpc.SwapState_SUCCESSFUL)

		require.Equal(t, info.ReverseSwap.State, lightzrpc.SwapState_SUCCESSFUL)
	...
	t.Run("ExternalPay", func(t *testing.T) {
		...
		getRequest := func() *lightzrpc.CreateReverseSwapRequest {
			return &lightzrpc.CreateReverseSwapRequest{
				Amount:         100000,
				AcceptZeroConf: true,
				ExternalPay:    &externalPay,
			}
		...
		t.Run("Description", func(t *testing.T) {
			...
			require.Equal(t, chainutil.Amount(request.Amount), decoded.MilliSat.ToLokis())
	...
	t.Run("ManualClaim", func(t *testing.T) {
		...
		createClaimable := func(t *testing.T) (*lightzrpc.ReverseSwapInfo, streamFunc, streamStatusFunc) {
			response, err := client.CreateWallet(&lightzrpc.WalletParams{
				Currency: lightzrpc.Currency_BTC,
				Name:     "temp",
			})
			require.NoError(t, err)
			swap, err := client.CreateReverseSwap(&lightzrpc.CreateReverseSwapRequest{
			...
			update := statusStream(lightzrpc.SwapState_ERROR, lightz.TransactionConfirmed)

			// we add a small sleep here to avoid the race where lightzd says confirmed but the chain provider hasn't synced
		...
		t.Run("Address", func(t *testing.T) {
			...
			destination := &lightzrpc.ClaimSwapsRequest_Address{}
			request := &lightzrpc.ClaimSwapsRequest{SwapIds: []string{info.Id}, Destination: destination}
			t.Run("Invalid", func(t *testing.T) {
				destination.Address = "invalid"
				_, err := client.ClaimSwaps(request)
				requireCode(t, err, codes.InvalidArgument)

				_, err = client.ClaimSwaps(&lightzrpc.ClaimSwapsRequest{SwapIds: []string{"invalid"}})
				requireCode(t, err, codes.NotFound)
			})
			...
			t.Run("Valid", func(t *testing.T) {
				...
				checkTxOutAddress(t, chain, lightz.CurrencyBtc, response.TransactionId, destination.Address, true)
		...
		t.Run("Wallet", func(t *testing.T) {
			...
			destination := &lightzrpc.ClaimSwapsRequest_WalletId{}
			request := &lightzrpc.ClaimSwapsRequest{SwapIds: []string{info.Id}, Destination: destination}
			...
			t.Run("Valid", func(t *testing.T) {
				destinationWallet := emptyWallet(t, client, lightzrpc.Currency_BTC)
...
func fundedWallet(t *testing.T, client client.Lightz, currency lightzrpc.Currency) *lightzrpc.Wallet {
	params := &lightzrpc.WalletParams{Currency: currency, Name: walletName(currency)}
	wallet, err := client.GetWallet(params.Name)
	if err != nil {
		mnemonic := test.WalletMnemonic
		subaccount := uint64(test.WalletSubaccount)
		creds := &lightzrpc.WalletCredentials{Mnemonic: &mnemonic, Subaccount: &subaccount}
...
func walletId(t *testing.T, client client.Lightz, currency lightzrpc.Currency) uint64 {
...
func emptyWallet(t *testing.T, client client.Lightz, currency lightzrpc.Currency) *lightzrpc.Wallet {
...
func TestAutoSwap(t *testing.T) {
	...
	defer stop()
	fundedWallet(t, admin, lightzrpc.Currency_LBTC)
	...
	t.Run("Chain", func(t *testing.T) {
		...
		cfg := &autoswaprpc.ChainConfig{
			FromWallet: walletName(lightzrpc.Currency_LBTC),
			ToWallet:   cfg.Node,
			Budget:     1_000_000,
		}
		...
		response, err := admin.ListSwaps(&lightzrpc.ListSwapsRequest{Include: lightzrpc.IncludeSwaps_AUTO})
		...
		stream(lightzrpc.SwapState_PENDING)
		test.MineBlock()
		stream(lightzrpc.SwapState_SUCCESSFUL)
	...
	t.Run("Lightning", func(t *testing.T) {
		...
		t.Run("Setup", func(t *testing.T) {
			...
			_, err = autoSwap.UpdateLightningConfig(&autoswaprpc.UpdateLightningConfigRequest{
				Config: &autoswaprpc.LightningConfig{
					Currency: lightzrpc.Currency_LBTC,
					Wallet:   walletName(lightzrpc.Currency_LBTC),
				},
		...
		t.Run("CantRemoveWallet", func(t *testing.T) {
			_, err := autoSwap.SetLightningConfigValue("wallet", walletName(lightzrpc.Currency_LBTC))
			require.NoError(t, err)
			_, err = autoSwap.SetLightningConfigValue("enabled", true)
			require.NoError(t, err)
			_, err = admin.RemoveWallet(walletId(t, admin, lightzrpc.Currency_LBTC))
			require.Error(t, err)
		})
		...
		t.Run("Start", func(t *testing.T) {
			...
			swapCfg.Currency = lightzrpc.Currency_BTC
			...
			t.Run("Recommendations", func(t *testing.T) {
				...
				require.Equal(t, lightzrpc.SwapType_REVERSE, recommendations.Lightning[0].Swap.Type)
			...
			t.Run("Auto", func(t *testing.T) {
				...
				swaps, err := admin.ListSwaps(&lightzrpc.ListSwapsRequest{Include: lightzrpc.IncludeSwaps_AUTO})
				require.NoError(t, err)
				// it might be the first index since we create swaps above aswell
				require.True(t, slices.ContainsFunc(swaps.ReverseSwaps, func(s *lightzrpc.ReverseSwapInfo) bool {
					return s.Id == id
				}))
				stream, _ = swapStream(t, admin, id)
				stream(lightzrpc.SwapState_SUCCESSFUL)
...
func TestWalletTransactions(t *testing.T) {
	...
	t.Run("Pagination", func(t *testing.T) {
		...
		response, err := client.ListWalletTransactions(&lightzrpc.ListWalletTransactionsRequest{
			Id:     walletInfo.Id,
			Offset: &offset,
			Limit:  &limit,
		})
	...
	t.Run("Readonly", func(t *testing.T) {
		...
		response, err := client.ListWalletTransactions(&lightzrpc.ListWalletTransactionsRequest{Id: walletInfo.Id})
		require.NoError(t, err)
	...
	tests := []struct {
		...
		fakeSwaps    test.FakeSwaps
		txType       lightzrpc.TransactionType
	}{
		{
			desc: "Claim",
			transactions: []*onchain.WalletTransaction{
			...
			txType: lightzrpc.TransactionType_CLAIM,
		},
		{
			desc: "Refund",
			transactions: []*onchain.WalletTransaction{
			...
			txType: lightzrpc.TransactionType_REFUND,
		},
		{
			desc: "Lockup",
			transactions: []*onchain.WalletTransaction{
			...
			txType: lightzrpc.TransactionType_LOCKUP,
		},

		{
			...
			txType: lightzrpc.TransactionType_CONSOLIDATION,
		},
	}
...
func TestBumpTransaction(t *testing.T) {
	...
	txIdRequest := &lightzrpc.BumpTransactionRequest{
		Previous: &lightzrpc.BumpTransactionRequest_TxId{TxId: someTxId},
	}
	...
	tests := []struct {
		...
		request *lightzrpc.BumpTransactionRequest
		wantErr string
	}{
		{
			desc:    "Success",
		...
		{
			desc: "SwapId/Success",
			request: &lightzrpc.BumpTransactionRequest{Previous: &lightzrpc.BumpTransactionRequest_SwapId{
				SwapId: "normal-swap",
			}},
			swaps: &test.FakeSwaps{
				Swaps: []database.Swap{
	...
	t.Run("Swaps", func(t *testing.T) {
		t.Run("Submarine", func(t *testing.T) {
			swap, err := client.CreateSwap(&lightzrpc.CreateSwapRequest{
				Amount: swapAmount,
			})
			require.NoError(t, err)
			...
			info := statusStream(lightzrpc.SwapState_PENDING, lightz.TransactionMempool)
		...
		t.Run("Chain", func(t *testing.T) {
			...
			swap, err := client.CreateChainSwap(&lightzrpc.CreateChainSwapRequest{
				Amount: &swapAmount,
				Pair: &lightzrpc.Pair{
					From: lightzrpc.Currency_BTC,
					To:   lightzrpc.Currency_LBTC,
				},
				ToAddress:   &toAddress,
				ExternalPay: &externalPay,
			})
			...
			info := statusStream(lightzrpc.SwapState_PENDING, lightz.TransactionMempool).GetChainSwap()
...
func TestWallet(t *testing.T) {
	...
	testWallet := fundedWallet(t, client, lightzrpc.Currency_LBTC)
	_, err := client.GetWalletCredentials(testWallet.Id, nil)
	require.NoError(t, err)

	walletParams := &lightzrpc.WalletParams{Currency: lightzrpc.Currency_LBTC, Name: "test", Password: &password}
	...
	mnemonic := "invalid"
	_, err = client.ImportWallet(walletParams, &lightzrpc.WalletCredentials{Mnemonic: &mnemonic})
	require.Error(t, err)
	...
	*/
	...
	_, err = client.GetWallet(walletName(lightzrpc.Currency_LBTC))
	require.NoError(t, err)
}
...
func TestUnlock(t *testing.T) {
	...
	walletCredentials := test.WalletCredentials(lightz.CurrencyBtc)
	...
	wrongPassword := "wrong"
	second := &lightzrpc.WalletParams{Currency: lightzrpc.Currency_LBTC, Name: "new", Password: &wrongPassword}
...
func TestDirectReverseSwapPayments(t *testing.T) {
	...
	chain := getOnchain(t, cfg)
	client, _, stop := setup(t, setupOptions{cfg: cfg, chain: chain})
	fundedWallet(t, client, lightzrpc.Currency_LBTC)
	defer stop()
	...
	t.Run("AddMagicRoutingHint", func(t *testing.T) {
		...
		wallet := emptyWallet(t, client, lightzrpc.Currency_LBTC)
		externalPay := true
		request.ExternalPay = &externalPay
		request.WalletId = &wallet.Id

		createAndCheckMrh := func() (*lightzrpc.ReverseSwapInfo, *btcec.PublicKey) {
			swap, err := client.CreateReverseSwap(request)
			require.NoError(t, err)
			...
			decoded, err := zpay32.Decode(*swap.Invoice, &chaincfg.RegressionNetParams)
			require.NoError(t, err)
			return swapInfo.ReverseSwap, lightz.FindMagicRoutingHint(decoded)
		}
	...
	tt := []struct {
		desc     string
		zeroconf bool
		currency lightzrpc.Currency
	}{
		{"Btc/Normal", false, lightzrpc.Currency_BTC},
		{"Liquid/Normal", false, lightzrpc.Currency_LBTC},
		{"Liquid/ZeroConf", true, lightzrpc.Currency_LBTC},
	}
	for _, tc := range tt {
		t.Run(tc.desc, func(t *testing.T) {

			confirmed := false
			if !tc.zeroconf || tc.currency == lightzrpc.Currency_BTC {
			...
			if !tc.zeroconf {
				if tc.currency == lightzrpc.Currency_BTC {
					// for btc, we only check on new blocks, so we mine one here,
					// but the mocked tx provider says that the tx isn't confirmed yet
					test.MineBlock()
				}
				statusStream(lightzrpc.SwapState_PENDING, lightz.TransactionDirectMempool)
			}

			confirmed = true
			test.MineBlock()
			info := statusStream(lightzrpc.SwapState_SUCCESSFUL, lightz.TransactionDirect)
...
func TestCreateWallet(t *testing.T) {
	...
	t.Run("WithPassword", func(t *testing.T) {
		...
		secondParams := &lightzrpc.WalletParams{Name: "another", Currency: lightzrpc.Currency_BTC, Password: &password}
	...
	t.Run("DuplicateName", func(t *testing.T) {
		_, err := client.CreateWallet(&lightzrpc.WalletParams{Name: walletName(lightzrpc.Currency_BTC), Currency: lightzrpc.Currency_BTC})
		require.Error(t, err)
	})
...
func TestWalletSendReceive(t *testing.T) {
	...
	t.Run("SendFee", func(t *testing.T) {
		...
		t.Run("NodeWallet", func(t *testing.T) {
			_, err := client.GetSendFee(&lightzrpc.WalletSendRequest{
				Id:            nodeWallet.Id,
				SendAll:       &sendAll,
		...
		t.Run("Normal", func(t *testing.T) {
			response, err := client.GetSendFee(&lightzrpc.WalletSendRequest{
				Id:            testWallet.Id,
				SendAll:       &sendAll,
				IsSwapAddress: &swapRelated,
			})
	...
	t.Run("Send", func(t *testing.T) {
		...
		defaultEstimate, err := chain.EstimateFee(lightz.CurrencyBtc)
		require.NoError(t, err)
...
func TestImportDuplicateCredentials(t *testing.T) {
	...
	second := &lightzrpc.WalletParams{Name: "another", Currency: lightzrpc.Currency_LBTC}
	_, err = client.ImportWallet(second, credentials)
	require.Error(t, err)

	second.Currency = lightzrpc.Currency_BTC
...
func TestSwap(t *testing.T) {
	...
	checkSwap := func(t *testing.T, swap *lightzrpc.SwapInfo) {
		...
		excpectedFees := swap.ExpectedAmount - uint64(invoice.MilliSat.ToLokis())
	...
	t.Run("Recovery", func(t *testing.T) {
		...
		for {
			...
			case <-ticker.C:
				info, err := client.GetSwapInfo(swap.Id)
				require.NoError(t, err)
				if info.Swap.State == lightzrpc.SwapState_SUCCESSFUL {
	...
	t.Run("IgnoreMrh", func(t *testing.T) {
		...
		reverseSwap, err := client.CreateReverseSwap(&lightzrpc.CreateReverseSwapRequest{
			ExternalPay: &externalPay,
			Amount:      100000,
			Pair: &lightzrpc.Pair{
				From: lightzrpc.Currency_BTC,
	...
	t.Run("Invoice", func(t *testing.T) {
		...
		t.Run("Offer", func(t *testing.T) {
			...
			require.NoError(t, err)
			_, statusStream := swapStream(t, client, swap.Id)
			info := statusStream(lightzrpc.SwapState_PENDING, lightz.InvoiceSet)
			require.NotEmpty(t, info.Swap.Invoice)
		})

		t.Run("Invalid", func(t *testing.T) {
			invoice := "invalid"
			_, err := client.CreateSwap(&lightzrpc.CreateSwapRequest{
				Invoice: &invoice,
			})
			require.Error(t, err)
		})
		...
		t.Run("LNURL", func(t *testing.T) {
			...
			t.Run("Valid", func(t *testing.T) {
				...
				require.NoError(t, err)
				require.Equal(t, amount, uint64(invoice.MilliSat.ToLokis()))
			})
		})
	})
	...
	for _, node := range nodes {
		...
		t.Run(node, func(t *testing.T) {
			...
			tests := []struct {
				desc string
				from lightzrpc.Currency
				cli  func(string) string
			}{
				{"Liquid", lightzrpc.Currency_LBTC, test.LiquidCli},
				{"BTC", lightzrpc.Currency_BTC, test.BtcCli},
			}
			...
			for _, tc := range tests {
				t.Run(tc.desc, func(t *testing.T) {
					...
					admin, _, stop := setup(t, setupOptions{cfg: cfg, lightzApi: lightzApi})
					defer stop()
					fundedWallet(t, admin, tc.from)

					submarinePair, err := admin.GetPairInfo(lightzrpc.SwapType_SUBMARINE, pair)
					require.NoError(t, err)
					...
					t.Run("AnyAmount", func(t *testing.T) {
						...
						statusStream(lightzrpc.SwapState_PENDING, lightz.InvoiceSet)
					...
					t.Run("Refund", func(t *testing.T) {
						...
						t.Run("Script", func(t *testing.T) {
							lightzApi.DisablePartialSignatures = true
							t.Cleanup(func() {
								lightzApi.DisablePartialSignatures = false
							})

							refundAddress := cli("getnewaddress")
							withStream, _ := createFailed(t, refundAddress)

							swap := withStream(lightzrpc.SwapState_ERROR).Swap

							test.MineUntil(t, cli, int64(swap.TimeoutBlockHeight))

							swap = withStream(lightzrpc.SwapState_REFUNDED).Swap
						...
						t.Run("Cooperative", func(t *testing.T) {
							...
							info := stream(lightzrpc.SwapState_REFUNDED).Swap
						...
						if tc.from == lightzrpc.Currency_BTC {
							t.Run("Manual", func(t *testing.T) {
								setup := func(t *testing.T) *lightzrpc.SwapInfo {
									_, statusStream := createFailed(t, "")
									info := statusStream(lightzrpc.SwapState_SERVER_ERROR, lightz.TransactionLockupFailed).Swap
								...
								t.Run("Address", func(t *testing.T) {
									info := setup(t)
									refundAddress := cli("getnewaddress")
									destination := &lightzrpc.RefundSwapRequest_Address{}
									request := &lightzrpc.RefundSwapRequest{Id: info.Id, Destination: destination}
									...
									t.Run("Invalid", func(t *testing.T) {
										...
										_, err = admin.RefundSwap(&lightzrpc.RefundSwapRequest{Id: "invalid"})
										requireCode(t, err, codes.NotFound)
									})
								...
								t.Run("Wallet", func(t *testing.T) {
									...
									destination := &lightzrpc.RefundSwapRequest_WalletId{}
									request := &lightzrpc.RefundSwapRequest{Id: info.Id, Destination: destination}
...
func TestChainSwap(t *testing.T) {
	...
	checkSwap := func(t *testing.T, client client.Lightz, id string) {
		response, err := client.ListSwaps(&lightzrpc.ListSwapsRequest{})
		require.NoError(t, err)
	...
	client, _, stop := setup(t, setupOptions{cfg: cfg, chain: chain, lightzApi: lightzApi, node: "standalone"})
	defer stop()
	...
	tests := []struct {
		...
		{"BTC", lightzrpc.Currency_BTC, lightzrpc.Currency_LBTC},
		{"Liquid", lightzrpc.Currency_LBTC, lightzrpc.Currency_BTC},
	}
	...
	t.Run("Recovery", func(t *testing.T) {
		...
		update := stream(lightzrpc.SwapState_SUCCESSFUL).ChainSwap
		require.Equal(t, swap.Id, update.Id)
		checkTxOutAddress(t, chain, lightz.CurrencyLiquid, update.ToData.GetTransactionId(), update.ToData.GetAddress(), true)
	})
	...
	t.Run("Quote", func(t *testing.T) {
		...
		to := test.LiquidCli("getnewaddress")
		pair := &lightzrpc.Pair{From: lightzrpc.Currency_BTC, To: lightzrpc.Currency_LBTC}
		pairInfo, err := client.GetPairInfo(lightzrpc.SwapType_CHAIN, pair)
		require.NoError(t, err)
		t.Run("WithinLimits", func(t *testing.T) {
			...
			info := statusStream(lightzrpc.SwapState_PENDING, lightz.TransactionLockupFailed)
		...
		t.Run("OutofLimits", func(t *testing.T) {
			...
			_, statusStream := swapStream(t, client, swap.Id)
			statusStream(lightzrpc.SwapState_SERVER_ERROR, lightz.TransactionLockupFailed)
		})
		...
		t.Run("InvalidServiceFee", func(t *testing.T) {
			...
			_, err = client.CreateChainSwap(&lightzrpc.CreateChainSwapRequest{
				Pair:           pair,
				ExternalPay:    &externalPay,
				ToAddress:      &to,
				AcceptZeroConf: &acceptZeroConf,
	...
	t.Run("Invalid", func(t *testing.T) {
		...
		toWallet := fundedWallet(t, client, lightzrpc.Currency_BTC)
		...
		for _, tc := range tests {
			t.Run(tc.desc, func(t *testing.T) {
				...
				swap, err := client.CreateChainSwap(
					&lightzrpc.CreateChainSwapRequest{
						Amount:      &swapAmount,
						ExternalPay: &externalPay,
						ToWalletId:  &toWallet.Id,
						Pair: &lightzrpc.Pair{
				...
				stream, statusStream := swapStream(t, client, swap.Id)
				statusStream(lightzrpc.SwapState_PENDING, lightz.TransactionServerMempoool)
				test.MineBlock()
				info := stream(lightzrpc.SwapState_ERROR)
	...
	t.Run("Retry", func(t *testing.T) {
		...
		acceptZeroConf := false
		externalPay := true
		toWallet := walletId(t, client, lightzrpc.Currency_BTC)
		request := &lightzrpc.CreateChainSwapRequest{
			Amount:         &swapAmount,
			Pair:           &lightzrpc.Pair{From: lightzrpc.Currency_LBTC, To: lightzrpc.Currency_BTC},
			AcceptZeroConf: &acceptZeroConf,
			ExternalPay:    &externalPay,
			ToWalletId:     &toWallet,
		}
		...
		test.MineBlock()
		// on first call, the broadcast will fail
		statusStream(lightzrpc.SwapState_ERROR, lightz.TransactionServerConfirmed)
		test.MineBlock()
		// new block triggers a retry, on which the broadcast will succeed
		statusStream(lightzrpc.SwapState_SUCCESSFUL, lightz.TransactionClaimed)
	...
	for _, tc := range tests {
		t.Run(tc.desc, func(t *testing.T) {
			...
			t.Run("External", func(t *testing.T) {
				...
				streamStatus(lightzrpc.SwapState_PENDING, lightz.TransactionMempool)
				test.MineBlock()
				streamStatus(lightzrpc.SwapState_PENDING, lightz.TransactionConfirmed)
				streamStatus(lightzrpc.SwapState_PENDING, lightz.TransactionServerMempoool)
				test.MineBlock()
				info := status(lightzrpc.SwapState_SUCCESSFUL).ChainSwap
			...
			t.Run("Refund", func(t *testing.T) {
				chainPair, err := client.GetPairInfo(lightzrpc.SwapType_CHAIN, pair)
				...
				t.Run("Script", func(t *testing.T) {
					...
					info := statusStream(lightzrpc.SwapState_ERROR, lightz.TransactionLockupFailed).ChainSwap
					test.MineUntil(t, fromCli, int64(info.FromData.TimeoutBlockHeight))
					info = stream(lightzrpc.SwapState_REFUNDED).ChainSwap
				...
				t.Run("Cooperative", func(t *testing.T) {
					...
					info := statusStream(lightzrpc.SwapState_REFUNDED, lightz.TransactionLockupFailed).ChainSwap
					require.Zero(t, info.ServiceFee)
				...
				if tc.from == lightzrpc.Currency_BTC {
					...
					t.Run("Manual", func(t *testing.T) {
						setup := func(t *testing.T) (*lightzrpc.ChainSwapInfo, streamStatusFunc) {
							_, statusStream := createFailed(t, "")
							info := statusStream(lightzrpc.SwapState_SERVER_ERROR, lightz.TransactionLockupFailed).ChainSwap
						...
						t.Run("Address", func(t *testing.T) {
							...
							t.Run("Invalid", func(t *testing.T) {
								...
								_, err = client.RefundSwap(&lightzrpc.RefundSwapRequest{Id: "invalid"})
								requireCode(t, err, codes.NotFound)
							})
						...
						t.Run("Wallet", func(t *testing.T) {
							...
							t.Run("Valid", func(t *testing.T) {
								...
								require.Eventually(t, func() bool {
									transactions, err := client.ListWalletTransactions(&lightzrpc.ListWalletTransactionsRequest{Id: fromWallet.Id})
									require.NoError(t, err)
									for _, transaction := range transactions.Transactions {
										for _, txInfo := range transaction.GetInfos() {
											if txInfo.Type == lightzrpc.TransactionType_REFUND && txInfo.GetSwapId() == info.Id {
												return true
											}
										}
									}
			...
			if tc.to == lightzrpc.Currency_BTC {
				t.Run("Manual", func(t *testing.T) {
					createClaimable := func(t *testing.T) (*lightzrpc.ChainSwapInfo, streamFunc, streamStatusFunc) {
						response, err := client.CreateWallet(&lightzrpc.WalletParams{
							Currency: tc.to,
							Name:     "temp",
						})
						...
						update := statusStream(lightzrpc.SwapState_ERROR, lightz.TransactionServerConfirmed)
					...
					t.Run("Wallet", func(t *testing.T) {
						...
						t.Run("Valid", func(t *testing.T) {
							...
							info = stream(lightzrpc.SwapState_SUCCESSFUL).ChainSwap
							checkSwap(t, client, info.Id)
...
func TestLegacyWallet(t *testing.T) {
	...
	rpcWallet := emptyWallet(t, client, lightzrpc.Currency_LBTC)
...
func TestSwapMnemonic(t *testing.T) {
	...
	mnemonic := "invalid"
	_, err = client.SetSwapMnemonic(&lightzrpc.SetSwapMnemonicRequest{
		Mnemonic: &lightzrpc.SetSwapMnemonicRequest_Existing{
			Existing: mnemonic,
		},
	})
	require.Error(t, err)
	...
	createSwap := func(t *testing.T, expectedKeyIndex uint32) {
		...
		privateKey, err := lightz.DeriveKey(mnemonic.Mnemonic, expectedKeyIndex)
		require.NoError(t, err)
	...
	response, err := client.SetSwapMnemonic(&lightzrpc.SetSwapMnemonicRequest{
		Mnemonic: &lightzrpc.SetSwapMnemonicRequest_Generate{
			Generate: true,
		},
	})
	require.NoError(t, err)
