...
func printPairs(pairs []*lightzrpc.PairInfo) {
...
func getIncludeSwaps(ctx *cli.Context) lightzrpc.IncludeSwaps {
	if ctx.Bool("manual") {
		return lightzrpc.IncludeSwaps_MANUAL
	}
	if ctx.Bool("auto") {
		return lightzrpc.IncludeSwaps_AUTO
	}
	return lightzrpc.IncludeSwaps_ALL
}
...
var getStatsCommand = &cli.Command{
	...
	Action: func(ctx *cli.Context) error {
		client := getClient(ctx)
		response, err := client.GetStats(&lightzrpc.GetStatsRequest{Include: getIncludeSwaps(ctx)})
		if err != nil {
			return err
		}
		if ctx.Bool("json") {
...
func listSwaps(ctx *cli.Context) error {
	client := getClient(ctx)
	unify := true
	request := &lightzrpc.ListSwapsRequest{
	...
	if ctx.Bool("pending") {
		...
		stateValue, ok := lightzrpc.SwapState_value[strings.ToUpper(state)]
		if !ok {
			return errors.New("invalid state")
		}
		state := lightzrpc.SwapState(stateValue)
		if ok {
			request.State = &state
		}
	}
	...
	if ctx.Bool("json") {
		...
		if len(list.AllSwaps) > 0 {
			tbl := table.New("Type", "ID", "From", "To", "State", "Status", "Send Amount", "Lightz Fee", "Network Fee", "Created At")
	...
	for {
		...
		if json {
			...
			if info.Swap != nil {
				...
				case lightzrpc.SwapState_ERROR:
					fmt.Printf("Error: %s\n", info.Swap.Error)
				case lightzrpc.SwapState_REFUNDED:
					fmt.Println("Swap was refunded")
				}

				status := lightz.ParseEvent(info.Swap.Status)
				switch status {
				...
				if swap.State == lightzrpc.SwapState_SUCCESSFUL {
					fmt.Printf("Paid %d sat onchain fee and %d sat lightzd fee\n", *info.Swap.OnchainFee, *info.Swap.ServiceFee)
				}
				...
				status := lightz.ParseEvent(swap.Status)
				switch status {
				...
				if swap.State == lightzrpc.SwapState_SUCCESSFUL {
					...
					if swap.ExternalPay || swap.RoutingFeeMsat == nil {
						...
						fmt.Printf("Paid %d msat routing fee, %d sat onchain fee and %d sat lightzd fee\n", *swap.RoutingFeeMsat, *swap.OnchainFee, *swap.ServiceFee)
					}
				}
				...
				case lightz.TransactionServerMempoool:
			...
			if state == lightzrpc.SwapState_SUCCESSFUL && !isGlobal {
				return nil
			}
			fmt.Println()
		}
...
func printStats(stats *lightzrpc.SwapStats) {
...
func askForWallet(ctx *cli.Context, message string, currency *lightzrpc.Currency, allowReadonly bool) (result *lightzrpc.Wallet, err error) {
	client := getClient(ctx)
	...
	var walletsByName = make(map[string]*lightzrpc.Wallet)
	...
	for _, wallet := range wallets.Wallets {
		...
		if wallet.Currency == lightzrpc.Currency_LBTC {
...
func autoSwapChainSetup(ctx *cli.Context) error {
	...
	toCurrency := lightzrpc.Currency_BTC
	if fromWallet.Currency == lightzrpc.Currency_BTC {
		toCurrency = lightzrpc.Currency_LBTC
	}
	...
	pairInfo, err := client.GetPairInfo(lightzrpc.SwapType_CHAIN, &lightzrpc.Pair{
...
var createSwapCommand = &cli.Command{
	...
	Description: "Creates a new swap (e.g. BTC -> Lightning) specifying the amount in satoshis.\n" +
		...
		"> lightzcli createswap btc 100000\n" +
		"Create a swap from liquid for any amount of satoshis that can be paid manually:\n" +
		"> lightzcli createswap --any-amount lbtc\n" +
		"Create a swap from mainchain using an existing invoice:\n" +
		"> lightzcli createswap --invoice  btc",
...
func createSwap(ctx *cli.Context) error {
	...
	pairInfo, err := client.GetPairInfo(lightzrpc.SwapType_SUBMARINE, pair)
	if err != nil {
		return err
	}
	...
	if swap.Id == "" {
		if externalPay {
			fmt.Println("This invoice is from a lightzd reverse swap. You can pay to the onchain address directly.")
...
var createChainSwapCommand = &cli.Command{
	...
	Description: "Creates a new chain swap (e.g. BTC -> L-BTC) specifying the amount in satoshis.\n" +
		...
		"\n> lightzcli createchainswap --from-wallet autoswap --to-wallet cold 100000" +
		"\nCreate a chain swap for 100000 satoshis from the L-BTC wallet 'autoswap' to a BTC address:" +
		"\n> lightzcli createchainswap --from-wallet autoswap --to-address bcrt1q0akydfs98pjmqqplz0kvaa5hphg237vcvgaez2 100000" +
		"\nCreate a chain swap for 100000 satoshis from BTC to the L-BTC wallet 'autoswap' which has to be paid manually:" +
		"\n> lightzcli createchainswap --from-external LBTC --to-wallet autoswap 100000",
...
func createChainSwap(ctx *cli.Context) error {
	...
	network, _ := lightz.ParseChain(info.Network)

	if toAddress := ctx.String("to-address"); toAddress != "" {
		to, err := lightz.GetAddressCurrency(network, toAddress)
		if err != nil {
			return err
		}
	...
	if refundAddress := ctx.String("refund-address"); refundAddress != "" {
		from, err := lightz.GetAddressCurrency(network, refundAddress)
		if err != nil {
			return err
		}
...
func refundSwap(ctx *cli.Context) error {
	...
	request := &lightzrpc.RefundSwapRequest{Id: id}
	walletId, err := getWalletId(ctx, destination)
	if err == nil {
		request.Destination = &lightzrpc.RefundSwapRequest_WalletId{WalletId: *walletId}
	} else {
		request.Destination = &lightzrpc.RefundSwapRequest_Address{Address: destination}
	}
...
func claimSwaps(ctx *cli.Context) error {
	client := getClient(ctx)
	request := &lightzrpc.ClaimSwapsRequest{SwapIds: ctx.Args().Tail()}
	address := ctx.Args().First()
	walletId, err := getWalletId(ctx, address)
	if err == nil {
		request.Destination = &lightzrpc.ClaimSwapsRequest_WalletId{WalletId: *walletId}
	} else {
		request.Destination = &lightzrpc.ClaimSwapsRequest_Address{Address: address}
	}
...
func parseCurrency(currency string) (lightzrpc.Currency, error) {
	if currency == "" {
		return lightzrpc.Currency_BTC, errors.New("currency is required, allowed values: BTC, LBTC")
	}
	upper := strings.ToUpper(currency)
	switch upper {
	case "LBTC", "L-BTC", "LIQUID":
		return lightzrpc.Currency_LBTC, nil
	case "BTC":
		return lightzrpc.Currency_BTC, nil
	}
	return lightzrpc.Currency_BTC, fmt.Errorf("invalid currency: %s, allowed values: BTC, LBTC", currency)
}
...
func createReverseSwap(ctx *cli.Context) error {
	...
	if !json {
		pairInfo, err := client.GetPairInfo(lightzrpc.SwapType_REVERSE, pair)
		if err != nil {
			return err
		}
...
var unlockCommand = &cli.Command{
	...
	Action: func(ctx *cli.Context) error {
		...
		fmt.Println("lightzd successfully unlocked!")
		return nil
	},
}
...
//nolint:staticcheck
func printSubaccount(info *lightzrpc.Subaccount) {
...
func walletParams(ctx *cli.Context) (*lightzrpc.WalletParams, error) {
	currency, err := parseCurrency(ctx.Args().Get(1))
	if err != nil {
		return nil, err
	}
	return &lightzrpc.WalletParams{
		Name:     ctx.Args().Get(0),
		Currency: currency,
	}, nil
}
...
func importWallet(ctx *cli.Context, params *lightzrpc.WalletParams, readonly bool) (wallet *lightzrpc.Wallet, err error) {
	client := getClient(ctx)
...
func createWallet(ctx *cli.Context, params *lightzrpc.WalletParams) (wallet *lightzrpc.Wallet, err error) {
	client := getClient(ctx)
...
func bumpFee(ctx *cli.Context) error {
	...
	if swapId := ctx.String("swap-id"); swapId != "" {
		request.Previous = &lightzrpc.BumpTransactionRequest_SwapId{SwapId: swapId}
	} else if txId := ctx.String("tx-id"); txId != "" {
		request.Previous = &lightzrpc.BumpTransactionRequest_TxId{TxId: txId}
	} else {
...
var formatMacaroonCommand = &cli.Command{
	Name:     "formatmacaroon",
	Category: "Debug",
	Usage:    "Formats the macaroon for connecting to lightzd-client in hex",
...
var shellCompletionsCommand = &cli.Command{
	...
	Action: func(ctx *cli.Context) error {
		...
		content := fmt.Sprintf("\n# load completions for lightzcli\nPROG=lightzcli source %s", scriptPath)
...
var bakeMacaroonCommand = &cli.Command{
	...
	Action: requireNArgs(1, func(ctx *cli.Context) error {
		client := getClient(ctx)
		request := &lightzrpc.BakeMacaroonRequest{}
		...
		for i := 0; i < args.Len(); i++ {
			switch args.Get(i) {
			case "read":
				request.Permissions = append(request.Permissions, &lightzrpc.MacaroonPermissions{
...
var swapMnemonicCommands = &cli.Command{
	...
	Subcommands: []*cli.Command{
		...
		},
		{
			...
			UsageText: "lightzcli swapmnemonic set \"abandon ... abandon about\"\nlightzcli swapmnemonic set --generate",
			Action: func(ctx *cli.Context) error {
				client := getClient(ctx)

				mnemonic := ctx.Args().First()
				generate := ctx.Bool("generate")
				request := &lightzrpc.SetSwapMnemonicRequest{}
				if generate {
					if mnemonic != "" {
						return errors.New("mnemonic cannot be provided when generating a new one")
					}
					request.Mnemonic = &lightzrpc.SetSwapMnemonicRequest_Generate{
						Generate: true,
					}
				} else {
					if mnemonic == "" {
					...
					request.Mnemonic = &lightzrpc.SetSwapMnemonicRequest_Existing{
						Existing: mnemonic,
					}
				}
