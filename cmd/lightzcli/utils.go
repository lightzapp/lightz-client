...
func printFees(fees *lightzrpc.SwapFees, amount uint64) {
	fmt.Println("The fees for this service are:")
	if amount == 0 {
		fmt.Printf("  - Lightz fee: %s%%\n", fmt.Sprint(fees.Percentage))
	} else {
		serviceFee := utils.Satoshis(lightz.CalculatePercentage(lightz.Percentage(fees.Percentage), amount))
		fmt.Printf("  - Lightz fee (%s%%): %s\n", fmt.Sprint(fees.Percentage), serviceFee)
	}
...
func printDeposit(amount uint64, address string, limits *lightzrpc.Limits) {
