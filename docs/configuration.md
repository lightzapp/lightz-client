...
Lightz Client can be configured via a `TOML` config file or CLI arguments. By
default, the config file is called `lightz.toml` and located in the data
directory of Lightz Client (`~/.lightzd` by default on Linux).
...
# Whether to use Lightz Pro fee rates
pro = false
...
[BOLTZ]
# By default the daemon automatically connects to the official Lightz Backend for the network your node is on
# This value is used to overwrite that
# url = "https://api.lightz.exchange"
