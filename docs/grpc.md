...
Paths for the REST proxy of the gRPC interface can be found [here](https://github.com/lightzapp/lightzd-client/blob/master/pkg/lightzrpc/rest-annotations.yaml).
...
**Deprecated.** Fetches the latest limits and fees from the Lightz backend API it is connected to.
...
**Deprecated.** This is a wrapper for channel creation swaps. The daemon only returns the ID, timeout block height and lockup address. The Lightz backend takes care of the rest. When an amount of onchain coins that is in the limits is sent to the address before the timeout block height, the daemon creates a new lightning invoice, sends it to the Lightz backend which will try to pay it and if that is not possible, create a new channel to make the swap succeed.
...
**Deprecated.** Create a new swap from onchain to a new lightning channel. The daemon will only accept the invoice payment if the HTLCs is coming trough a new channel channel opened by Lightz.
...
Creates a new reverse swap from lightning to onchain. If `accept_zero_conf` is set to true in the request, the daemon will not wait until the lockup transaction from Lightz is confirmed in a block, but will claim it instantly.
...
Creates a new chain swap from one chain to another. If `accept_zero_conf` is set to true in the request, the daemon will not wait until the lockup transaction from Lightz is confirmed in a block, but will claim it instantly.
...
| `accept_zero_conf` | [`bool`](#bool) | optional | Whether the daemon should broadcast the claim transaction immediately after the lockup transaction is in the mempool. Should only be used for smaller amounts as it involves trust in Lightz. |
...
| `accepted_pair` | [`PairInfo`](#pairinfo) | optional | Rates to accept for the swap. Queries latest from lightz otherwise The recommended way to use this is to pass a user approved value from a previous `GetPairInfo` call |
...
| `accept_zero_conf` | [`bool`](#bool) |  | Whether the daemon should broadcast the claim transaction immediately after the lockup transaction is in the mempool. Should only be used for smaller amounts as it involves trust in lightz. |
| `pair` | [`Pair`](#pair) |  |  |
...
| `state` | [`SwapState`](#swapstate) |  |  |
| `error` | [`string`](#string) |  |  |
| `status` | [`string`](#string) |  | Latest status message of the Lightz backend |
| `private_key` | [`string`](#string) |  |  |
| `preimage` | [`string`](#string) |  |  |
...
| `start_date` | [`int64`](#int64) |  |  |
| `end_date` | [`int64`](#int64) |  |  |
| `stats` | [`lightzrpc.SwapStats`](#lightzrpc.swapstats) | optional |  |
...
| `wallet_balance` | [`lightzrpc.Balance`](#lightzrpc.balance) |  |  |
| `max_balance` | [`uint64`](#uint64) |  |  |
...
| `budget` | [`uint64`](#uint64) |  |  |
| `budget_interval` | [`uint64`](#uint64) |  |  |
| `currency` | [`lightzrpc.Currency`](#lightzrpc.currency) |  |  |
...
| `channel` | [`lightzrpc.LightningChannel`](#lightzrpc.lightningchannel) |  |  |
...
| `amount` | [`uint64`](#uint64) |  |  |
| `fee_estimate` | [`uint64`](#uint64) |  |  |
| `type` | [`lightzrpc.SwapType`](#lightzrpc.swaptype) |  |  |
| `dismissed_reasons` | [`string`](#string) | repeated | Reasons for which the swap is not being executed |
