...
The autoswap feature of `lightzd-client` allows for automated channel rebalancing.

## Setup

The `lightzcli autoswap setup` command will guide you through the initial
configuration of autoswap.

## Advanced Configuration

Advanced configuration that is not covered in the setup process can be managed
using `lightzcli autoswap config`. For instance, run
`lightzcli autoswap config acceptZeroConf true` to enable
[0-conf swaps](https://docs.lightz.exchange/v/api/0-conf). Optionally, you can
...
via `lightzcli autoswap config --reload`.
...
current configuration, using `lightzcli autoswap recommendations.`
...
available wallets using `lightzcli wallet list`. Note that the wallet currency
